...
if [ "${#}" -ne 2 ]; then
	echo "Usage: ${0} <image_repository> <${__TEMPLATE_TOOL_NAME__}_version>"
	exit 1
fi;
...
export REPOSITORY_DESCRIPTION="image bundling ${__TEMPLATE_TOOL_NAME__} and ... , built everyday by github-actions"
...
${__TEMPLATE_TOOL_NAME_CAPS__}_VERSION="${2}"

IMAGE_NAME=$(echo "${IMAGE_REPOSITORY}" | rev | cut -d '/' -f 1 | rev)
export IMAGE_NAME
export IMAGE_TAG="${${__TEMPLATE_TOOL_NAME_CAPS__}_VERSION}"
...
docker build \
	...
	-f Dockerfile \
	--build-arg ${__TEMPLATE_TOOL_NAME_CAPS__}_VERSION="${${__TEMPLATE_TOOL_NAME_CAPS__}_VERSION}" \
	.
...
make -C "${ROOT_DIR_LOCATION_}" ${__TEMPLATE_IMAGE_NAME__}-tests
r=$?
if [ $r -eq 0 ];
then
